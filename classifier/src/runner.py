    ...
    def load_x_train(self) -> np.array:
        ...
        if self.train_x is None:
            if self.features == "bow":
                matrix = np.load('../vec/bow_train_x.npy', allow_pickle = True)
                self.train_x = sparse.csr_matrix(matrix, dtype=np.float64)
            elif self.features == "bow_nva":
                matrix = np.load('../vec/bow_train_x_nva.npy', allow_pickle = True)
                self.train_x = sparse.csr_matrix(matrix, dtype=np.float64)
            elif self.features == "bow_tf-idf":
                matrix = np.load('../vec/bow_train_x_tf-idf.npy', allow_pickle = True)
                self.train_x = sparse.csr_matrix(matrix, dtype=np.float64)
            elif self.features == "term_2-gram":
                matrix = np.load('../vec/bow_train_x_2-gram.npy', allow_pickle = True)
                self.train_x = sparse.csr_matrix(matrix, dtype=np.float64)
